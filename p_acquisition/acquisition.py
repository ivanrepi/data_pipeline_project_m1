...
def acquisition_json(path):
    try:
        ...
        df = pd.json_normalize(df['@graph']) #Only needed the info inside @graph
        ...
        df=pd.read_csv("data/raw/instalaciones.csv", sep=";").drop_duplicates() #Return CSV lready created if connection fails
        return df


def get_station_details(df,place):
    instalacion_n= df.loc[df["title"] == place] #Get the row where the place of interest is the choosen one
    station_id = instalacion_n["id_y"].to_string(index=False) #Get the bicimad station id
    header={'email':os.getenv('emt_madrid_email'),'password':os.getenv('emt_madrid_pwd')} 
    url='https://openapi.emtmadrid.es/v1/mobilitylabs/user/login/'
    get_token=requests.get(url, headers=header).json() #Get the token from the Api before
    accessToken=get_token['data'][0]['accessToken'] #Token
    ...
    return (my_dataset['data'][0]) #Return the information from the statio selected
